...
class GreedyBase(BaseEstimator, PythonModel):
    ...
    #         candidate_rules, key=lambda r: self.objective_of_rule(r), reverse=True
    #     )

    #     # 選取前 k 個最佳候選規則
    #     k = min(5, len(sorted_candidates))  # 您可以根據需要調整 k 的值
    #     top_k_candidates = sorted_candidates[:k]

    #     # 從前 k 個中隨機選擇一個
    #     best_candidate = random.choice(top_k_candidates)
    #     return best_candidate
    
    # dany V6
    # def select_best_candidate(self, candidate_rules):
    #     best_candidate = max(candidate_rules, key=lambda r: self.objective_of_rule(r))
    ...
    #     # 選取前 K% 的最佳候選規則
    #     K = 0.6  # 例如，選擇前 V3(50%) V4(70%) V6(60%)的規則
    #     k = max(1, int(len(sorted_candidates) * K))  # 確保至少選擇一個規則
    #     top_k_candidates = sorted_candidates[:k]

    #     return best_candidate, top_k_candidates
    
    # dany V6
    def select_best_candidate_2(self, candidate_rules):
        return max(candidate_rules, key=lambda r: self.objective_of_rule(r))
