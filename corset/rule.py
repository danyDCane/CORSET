...
class Rule:
    ...
    def marginal_coverage(self, ref_rules):
        ...
        # 合併所有 ref_rules 的覆蓋位置
        ref_label_occurences = set()
        for ref_rule in ref_rules:
            ref_label_occurences |= extract_label_occs(ref_rule)

        # 一次性進行減法操作
        current_label_occurences -= ref_label_occurences

        uncovered_label_area = len(current_label_occurences)

        if uncovered_label_area < 0:
            raise Exception(f'something terrible goes wrong: `uncovered_label_area={uncovered_label_area}` cannot be negative')
