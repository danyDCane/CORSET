...
def prune_edges(g: nx.Graph, min_proba: float = None, proba_key='proba', dynamic_threshold=False):
    """
    Prune edges in the graph based on a static or dynamically adjusted threshold.
   
    Args:
        g (nx.Graph): Input graph.
        min_proba (float): Static threshold for pruning edges. Used if `dynamic_threshold` is False.
        proba_key (str): Edge attribute key representing the probability.
        dynamic_threshold (bool): Whether to use a dynamically adjusted threshold.
    ...
    if dynamic_threshold:
        edge_weights = [g[u][v][proba_key] for u, v in g.edges()]
        # 動態設置 min_proba 為邊權重的 75% 分位數（可調整策略）
        min_proba = np.percentile(edge_weights, 75)
        logger.debug(f"Dynamically adjusted threshold (min_proba): {min_proba}")
