...
class UncoveredAreaMixin:
    ...
    #         )
    #         # 新增：引入多樣性特徵
    #         diversity_weight = self._calculate_diversity(row)
    #         # 新增：引入重要性特徵
    #         importance_weight = self._calculate_importance(row)
            
    #         # 新增：總權重
    #         total_weight = uncovered_weight + diversity_weight + importance_weight
            
    #         # 新增：調試信息
    #         print(f"Row {i}: uncovered_weight={uncovered_weight}, diversity_weight={diversity_weight}, importance_weight={importance_weight}, total_weight={total_weight}")
            
    #         self.row_weights[i] = total_weight
        
    #     self._update_row_probas()
    #     self._build_element_counter()

    # 新增：計算樣本的多樣性
    def _calculate_diversity(self, row):
        """計算樣本的多樣性"""
        unique_elements = set()
        for sample_id in row:
            unique_elements.update(self._sample_space[sample_id])
        return len(unique_elements)

    # 新增：計算樣本的重要性
    def _calculate_importance(self, row):
        """計算樣本的重要性"""
        importance = 0
        for sample_id in row:
            importance += len(self._sample_space[sample_id])
        return importance
    ...
    #     # 新增：計算樣本的未覆蓋區域權重
    #     uncovered_weight = sample_weights.sum()
        
    #     # 新增：計算多樣性和重要性特徵
    #     diversity_weight = self._calculate_diversity(sample_ids)
    #     importance_weight = self._calculate_importance(sample_ids)
        
    #     # 新增：總權重
    #     total_weight = uncovered_weight + diversity_weight + importance_weight

    #     if not np.allclose(total_weight, self.row_weights[row_id]):
    #         print("row_id: ", row_id)
    #         print('row samples', [self._sample_space[sample_id] for sample_id in sample_ids])
    #         print('self.update_history', self.update_history)
    #         raise Exception(
    #             "sample_weights sum does not equal the DR's weight: {} != {}".format(
    #                 total_weight, self.row_weights[row_id]
    #             )
    #         )
